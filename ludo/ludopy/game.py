...
class Game:
    ...
    def __init__(self, ghost_players=None, seed=None):
        """
    ...
    def _dice_generator(self):
        ...
        """
        self.current_dice = self.rng.integers(1, 7)  # 7 is not included
    ...
    def get_observation(self, forced_dice=None):
        """
        ...
        if forced_dice is not None:
            self.current_dice = forced_dice
            obs = self._gen_observation(self.current_player, roll_dice=False)
        else:
    ...
    def _count_player(self):
        ...
        # If the count is over 3 then move to player 0 and count up the round
        if self.current_player > 3:
            self.current_player = 0
            self.round += 1

    def answer_observation(self, piece_to_move, my_seed=None):
        """
